# -*- coding: utf-8 -*-
"""
1_Data_Center/Data_Operations/Trade_Logger_FTMO_51512388519.py
...
- Speichert inkrementell (dedup) in:
...
- Speichert erst, wenn Trade geschlossen ist (Exit-Deal vorhanden)

Install:
  pip install MetaTrader5 pandas numpy

Start:
  python 1_Data_Center/Data_Operations/Trade_Logger_FTMO_51512388519.py
...
# ============================================================
# CONFIG (Account 51512388519)
# ============================================================

FIXED_MT5_DIR = Path(
    r"C:\Users\Leon\Desktop\MetaTrader 5 - Kopie - Kopie - Kopie (17) - Kopie - Kopie - Kopie - Kopie"
)

MT5_LOGIN = int(os.getenv("MT5_LOGIN", "51512388519"))
MT5_PASSWORD = os.getenv("MT5_PASSWORD", "g8F??C2!T")  # besser: ENV setzen
...
POLL_OVERLAP_SECONDS = 300        # 5 Minuten Overlap (sicher gegen Verzögerungen)
...
def build_closed_trades_from_deals(df: pd.DataFrame) -> pd.DataFrame:
    """
    Output: 1 Zeile pro geschlossener position_id, inkl. close_ticket (Dedup Key).
    """
    if df.empty:
        return pd.DataFrame()
    ...
    for pos_id, g in df.groupby("position_id", sort=False):
        ...
        if g_out.empty:
            continue
        if g_in.empty:
            # IN-Deal liegt evtl. außerhalb des Fensters; Backfill deckt das normalerweise ab
            continue
        ...
        closed_rows.append({
            ...
            "close_ticket": int(close_ticket),  # Dedup key
        })
