# -*- coding: utf-8 -*-
"""
QUANT/Data_Center/Backend_Management/1_Pipelines/Market/Ohcl_Logger/code.py
...
Output-Struktur:
QUANT/Data_Center/Data/1_Pipeline/Market/ohcl/<TF>/<SYMBOL>.parquet
QUANT/Data_Center/Data/1_Pipeline/Market/ohcl/summary.json

Wichtig:
- Kein zusätzlicher Unterordner OHCL unter Raw
...
# CODE_REGISTRY
# ============================================================
script_id: market_ohlc_mt5_loader
script_name: code.py
owner: Leon Everts
...
purpose:
- Lädt OHLC-Historie aus MetaTrader 5, speichert sie als Parquet-Dateien
  pro Symbol/Timeframe und aktualisiert bestehende Historien inkrementell.
- Baut beschädigte oder zu kurze Historien automatisch neu auf.
- Erstellt zusätzliche Q- und Y-Zeitreihen aus MN1-Daten.
- Schreibt technische Metadaten direkt in die Parquet-Dateien und summary.json.

inputs:
- MetaTrader 5 Terminal über festen terminal64.exe-Pfad
- MT5 Login/Server/Passwort aus ENV oder Default-Konfiguration
- SYMBOLS und BASE_TIMEFRAMES aus diesem Script

outputs:
- Data_Center/Data/1_Pipeline/Market/ohcl/<TF>/<SYMBOL>.parquet
- Data_Center/Data/1_Pipeline/Market/ohcl/summary.json

upstream_data:
- MT5 Broker-Historie über copy_rates_range / copy_rates_from_pos
...
environment: desktop
registry_group: data_management
author: Leon Everts
reviewer: ChatGPT
created_date: 2026-05-30

change_log:
- v1.0.0 - Initial OHLC MT5 Loader
- v1.1.0 - Added CODE_REGISTRY metadata block and runtime registry object
- v1.2.0 - Adapted root detection and output paths to QUANT structure
...
notes:
- Output root is Data_Center/Data/1_Pipeline/Market/ohcl.
- No additional OHCL folder below raw.
- Existing histories shorter than REQUIRED_HISTORY_START_UTC are rebuilt.
# ============================================================
"""
...
CODE_REGISTRY: Dict[str, object] = {
    ...
    "asset_type": "Live",
    "purpose": (
        "Loads OHLC history from MetaTrader 5, stores per-symbol/per-timeframe "
        "Parquet files, rebuilds incomplete histories, performs incremental updates, "
        "and derives Q/Y candles from MN1 data."
    ),
    "inputs": [
        "MetaTrader 5 terminal64.exe",
        "MT5 account credentials from environment/default config",
        "Configured SYMBOLS list",
        "Configured BASE_TIMEFRAMES list",
    ],
    "outputs": [
        "Data_Center/Data/1_Pipeline/Market/ohcl/<TF>/<SYMBOL>.parquet",
        "Data_Center/Data/1_Pipeline/Market/ohcl/summary.json",
    ...
    "upstream_data": [
        "MT5 broker OHLC history via copy_rates_range",
        "MT5 latest bars via copy_rates_from_pos",
    ],
    "downstream_data": [
        "Feature Engineering",
        "Regime Analysis",
    ...
    "notes": [
        "Output root is Data_Center/Data/1_Pipeline/Market/ohcl.",
        "Existing histories shorter than REQUIRED_HISTORY_START_UTC are rebuilt.",
        "Q and Y candles are derived from MN1 data.",
    ],
}
...
def find_quant_root(start: Path) -> Path:
    """
    Erwartet neue QUANT-Root-Struktur:
    ...
    for p in [cur] + list(cur.parents):
        ...
        backend_dir = data_center / "Backend_Management"
        dashboard_dir = p / "Dashboard"

        if (
            dashboard_dir.exists()
            and data_center.exists()
            and data_dir.exists()
...
# Backward-compatible alias, damit bestehende Prints/Logik nicht brechen.
...
OUT_DIR = MARKET_DATA_DIR / "ohcl"

# Backward-compatible aliases for existing log output.
DATA_OPERATIONS_DIR = BACKEND_MANAGEMENT_DIR
RAW_DIR = PIPELINE_DATA_DIR
MARKET_RAW_DIR = MARKET_DATA_DIR
...
def main() -> None:
    ...
    print(f"[INFO] DATA_CENTER_DIR             = {DATA_CENTER_DIR}")
    print(f"[INFO] BACKEND_MANAGEMENT_DIR      = {BACKEND_MANAGEMENT_DIR}")
    print(f"[INFO] PIPELINES_DIR               = {PIPELINES_DIR}")
    print(f"[INFO] PIPELINE_DATA_DIR           = {PIPELINE_DATA_DIR}")
    print(f"[INFO] MARKET_DATA_DIR             = {MARKET_DATA_DIR}")
