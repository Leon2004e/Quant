//+------------------------------------------------------------------+
//| Strategy 5.51.104 EA
//|
//|    Generated by StrategyQuant X Build 142 for MetaTrader
//|    Generated at 04/04/2026 06:38
//| 
//|    Backtested on FTMO_GBPJPY_M15 / M15, 2021.07.15 - 2026.03.31 
//+------------------------------------------------------------------+
...
input string CustomComment = "Strategy 5.51.104";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 0;        //MagicNumber
input int EMAPeriod1 = 30;        //EMAPeriod1
input int ATRPeriod1 = 14;        //ATRPeriod1
input int Period1 = 40;        //Period1
input int ATRPeriod2 = 146;        //ATRPeriod2
input double ProfitTargetCoef1 = 2.1;        //ProfitTargetCoef1
input double StopLossCoef1 = 6.8;        //StopLossCoef1
...
input double InitialCapital = 200000; //InitialCapital (0 means whole account balance). Used in some MM methods.
...
#define EMA_1 0     //iMA(NULL,0, EMAPeriod1, 0, MODE_EMA, PRICE_CLOSE)
#define ATR_1 1     //iCustom(NULL,0, "SqATR", ATRPeriod1)
#define FIBO_1 2     //iCustom(NULL,0, "SqFibo", 5, 0, 23.6, 0)
#define LOWEST_1 3     //iCustom(NULL,0, "SqLowest", Period1, PRICE_WEIGHTED)
#define TRUERANGE_1 4     //iCustom(NULL,0, "SqTrueRange");
#define ATR_2 5     //iCustom(NULL,0, "SqATR", ATRPeriod2)
#define ATR_3 6     //iCustom(NULL, 0, "SqATR", 30)
...
string sqStrategyName = "Strategy 5.51.104";
...
//------------------------
   if (_sqIsBarOpen == true
      &&   (indyCrossesBelow("sqClose(NULL,0, 1)", "(sqGetIndicatorValue(EMA_1, 0, 1) + (sqGetIndicatorValue(ATR_1, 0, 1) * 1.5))", 0, 0, 0, 0)
      &&   ((NormalizeDouble((double) sqGetIndicatorValue(FIBO_1, 0, 0), 6) > NormalizeDouble((double) sqGetIndicatorValue(LOWEST_1, 0, 3), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(TRUERANGE_1, 0, 2), 6) < NormalizeDouble((double) sqGetIndicatorValue(ATR_2, 0, 3), 6))))
)
   {
      // Action #1
        // Enter at Market
      openPrice = 0;  //Open at current market price
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY, openPrice, 2, StopLossCoef1 * sqGetIndicatorValue(ATR_3, 1)), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY, openPrice, 2, ProfitTargetCoef1 * sqGetIndicatorValue(ATR_3, 1)), "Current");
      size = sqMMRiskFixedAccountPct("Current",ORDER_TYPE_BUY,openPrice,sl,mmRiskPercent,mmDecimals,mmStopLossPips,mmLotsIfNoMM,mmMaxLots,mmMultiplier,mmStep);
...
bool initIndicators(){
   ...
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[FIBO_1] = iCustom(NULL,0, "SqFibo", 5, 0, 23.6, 0);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWEST_1] = iCustom(NULL,0, "SqLowest", Period1, PRICE_WEIGHTED);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[TRUERANGE_1] = iCustom(NULL,0, "SqTrueRange");;
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_2] = iCustom(NULL,0, "SqATR", ATRPeriod2);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_3] = iCustom(NULL, 0, "SqATR", 30);
...
bool isFibo(uchar indyIndex){
    if(indyIndex == FIBO_1) return true;
   return false;
}
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("ProfitTargetCoef1 * sqGetIndicatorValue(ATR_3, 1)")) {
      return (ProfitTargetCoef1 * sqGetIndicatorValue(ATR_3, 1));
   }
...
double sqGetExpressionByIdentification(string id, int shift) {
      if(id == "sqClose(NULL,0, 1)") {
          return   sqClose(NULL,0, shift + 1);
      }
      if(id == "(sqGetIndicatorValue(EMA_1, 0, 1) + (sqGetIndicatorValue(ATR_1, 0, 1) * 1.5))") {
          return   (sqGetIndicatorValue(EMA_1, 0, shift + 1) + (sqGetIndicatorValue(ATR_1, 0, shift + 1) * 1.5));
      }
