...
# CODE_REGISTRY
# ============================================================
# script_id: data_catalog_tree_selector
# script_name: Data Catalog Tree Selector
# owner: Leon
# status: active
# layer: Dashboard
# domain: Data Catalog
# asset_type: Dashboard Building Block
# purpose: Minimal Data Catalog tree/actions panel. Shows selectable data tree, actions, output preview, and can create real folders/data files inside Data_Center/Data. Newly created folders/files appear immediately even before catalog scanner updates catalog.db.
# inputs:
#   - Data_Center/Data/5_Catalog/catalog.db
#   - Data_Center/Data filesystem
# outputs:
#   - Dashboard UI
# dependencies:
#   - tkinter
...
#   - pathlib
# schedule: manual
# version: v1.7.1_new_folder_new_data_file_visible
...
def stage_from_path(rel_path: str) -> str:
    first = norm_path(rel_path).split("/")[0] if rel_path else ""
    mapping = {
        "1_Pipeline": "Pipeline",
        "2_Baseline": "Baseline",
        "3_Research": "Research",
...
class DataCatalogTreeRepository:
        ...
        if self.db_path.exists():
            try:
                with sqlite3.connect(self.db_path) as conn:
                    db_df = pd.read_sql_query("SELECT * FROM data_assets ORDER BY relative_path", conn)
                ...
                merged["_source_priority"] = merged["quality_status"].astype(str).map(lambda x: 0 if x and x != "filesystem" else 1)
                ...
                messagebox.showerror("Catalog DB Error", str(exc))
    ...
    def _fallback_from_filesystem(self) -> pd.DataFrame:
        ...
        allowed = {".parquet", ".csv", ".json", ".db", ".xlsx", ".txt", ".py", ".md", ".yaml", ".yml"}
        idx = 1
        ...
        for p in DATA_DIR.rglob("*"):
            ...
            rows.append({
                ...
                "domain": rel.split("/")[1] if len(rel.split("/")) > 1 else "",
                "asset_type": file_type_from_name(p.name),
                "account_type": "",
                "symbol": "",
                "strategy_id": "",
                "sample_type": "",
                "rows_count": "",
                "columns_count": "",
                "quality_status": "filesystem",
                "quality_score": "",
                "quality_message": "",
                "file_size_mb": round(stat.st_size / 1024 / 1024, 6),
                "date_start": "",
                "date_end": "",
                "last_modified_utc": "",
                "scanned_at_utc": "",
    ...
    def _clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        d["stage"] = d["stage"].fillna("").astype(str)
        d.loc[d["stage"].str.strip() == "", "stage"] = d["relative_path"].map(stage_from_path)
    ...
    def load_real_folders(self) -> list[str]:
        ...
        for p in DATA_DIR.rglob("*"):
            ...
            if any(part in skip_names for part in p.parts):
                continue
            try:
                folders.add(norm_path(p.relative_to(DATA_DIR).as_posix()))
            except Exception:
                pass
...
class DataCatalogTreeSelector(tk.Frame):
    def __init__(self, parent, repository: Optional[DataCatalogTreeRepository] = None, **kwargs):
        super().__init__(parent, bg=APP_BG, **kwargs)
        self.repo = repository or DataCatalogTreeRepository()
        ...
        self._direct_assets_cache: dict[str, pd.DataFrame] = {}
    ...
    def _build_header(self):
        ...
        tk.Label(header, text="DATA CATALOG", bg=APP_BG, fg=TEXT, font=FONT_TITLE).grid(row=0, column=0, sticky="w")
    ...
    def _build_actions(self):
        ...
        actions = [
            ...
            ("New Data File", self.new_data_file),
    ...
    def rebuild_indexes(self):
        ...
        if not self.df_all.empty:
            for folder, group in self.df_all.groupby(self.df_all["folder_path"].astype(str).map(norm_path), dropna=False):
                folder = norm_path(folder)
                self._direct_assets_cache[folder] = group.copy()
                if folder:
                    parts = [p for p in folder.split("/") if p]
                    for i in range(1, len(parts) + 1):
                        folder_set.add("/".join(parts[:i]))
    ...
    def apply_search(self, redraw: bool = True):
        ...
        if q and not d.empty:
            ...
            for col in [
                "file_name", "relative_path", "file_path", "stage", "domain",
                "asset_type", "symbol", "quality_status", "columns_list",
            ]:
                if col in d.columns:
    ...
    def direct_assets(self, folder: str) -> pd.DataFrame:
        return self._direct_assets_cache.get(norm_path(folder), pd.DataFrame()).copy()
    ...
    def draw_tree(self):
        ...
        y = 10
        y = self._draw_folder_row("", "Data", y, level=0, root=True)
    ...
    def _draw_folder(self, folder: str, y: int, level: int) -> int:
        ...
        if folder in self.expanded_folders:
            ...
            direct = self.direct_assets(folder)
            for _, row in direct.head(500).iterrows():
                y = self._draw_file_row(row, y, level + 1)

            hidden = len(direct) - 500
            if hidden > 0:
                c = self.tree_canvas
                x = 44 + (level + 1) * INDENT
                c.create_text(x, y + TREE_ROW_H / 2, text=f"+ {hidden:,} more files", anchor="w", fill=MUTED, font=FONT_SMALL)
                y += TREE_ROW_H
    ...
    def _draw_folder_row(self, folder: str, name: str, y: int, level: int, root: bool = False) -> int:
        ...
        selected = self.selected_asset_id is None and self.selected_folder == folder
        ...
        self._bind_area(8, y, w, y + h, kind="folder", folder=folder, asset_id=None)
    ...
    def _draw_file_row(self, row: pd.Series, y: int, level: int) -> int:
        ...
        selected = self.selected_asset_id == fid
        checked = fid in self.marked_assets
        ...
        qc = quality_color(row.get("quality_status"))

        c.create_text(x + 28, y + h / 2, text=file_icon(name), anchor="w", fill=qc, font=FONT_SMALL)
        c.create_text(x + 52, y + h / 2, text=name, anchor="w", fill=TEXT, font=FONT_BODY)
        c.create_text(w - 16, y + h / 2, text=safe_str(row.get("asset_type")) or file_type_from_name(name), anchor="e", fill=MUTED, font=FONT_SMALL)

        self._bind_area(8, y, w, y + h, kind="file", folder=norm_path(row.get("folder_path")), asset_id=fid)
        self._tree_y_to_item.append((y, y + h, "file", norm_path(row.get("folder_path")), fid))
    ...
    def _bind_area(self, x1: int, y1: int, x2: int, y2: int, kind: str, folder: str, asset_id: Optional[int]):
        c = self.tree_canvas
        ...
        if kind == "folder":
            ...
            c.tag_bind(hit, "<Button-1>", lambda e, fid=asset_id: self._file_click(e, fid))
            c.tag_bind(hit, "<Double-1>", lambda e, fid=asset_id: self.open_file(fid))
    ...
    def _file_click(self, event, asset_id: Optional[int]):
        if asset_id is None:
            return
        row = self.row_by_id(asset_id)
        if row is None:
            return
        ...
        if checkbox_x - 3 <= x <= checkbox_x + CHECK_SIZE + 3:
            self.toggle_file_mark(asset_id)
        else:
            self.select_file(asset_id)
    ...
    def select_folder(self, folder: str):
        ...
        self.selected_asset_id = None
        self.status_var.set(f"Selected folder: {self.selected_folder or 'Data'}")
        self.draw_tree()

    def select_file(self, asset_id: int):
        self.selected_asset_id = int(asset_id)
        row = self.row_by_id(asset_id)
        self.selected_folder = norm_path(row.get("folder_path")) if row is not None else ""
        self.status_var.set(f"Selected file: {safe_str(row.get('relative_path')) if row is not None else asset_id}")
        self.draw_tree()
    ...
    def toggle_file_mark(self, asset_id: int):
        asset_id = int(asset_id)
        if asset_id in self.marked_assets:
            self.marked_assets.remove(asset_id)
        else:
            self.marked_assets.add(asset_id)
    ...
    def _mark_subtree(self, folder: str):
        ...
        for _, row in self.direct_assets(folder).iterrows():
            self.marked_assets.add(int(row.get("id")))
    ...
    def _unmark_subtree(self, folder: str):
        ...
        for _, row in self.direct_assets(folder).iterrows():
            self.marked_assets.discard(int(row.get("id")))
    ...
    def update_selection_state(self):
        total = len(self.marked_folders) + len(self.marked_assets)
    ...
    def select_all(self):
        for folder in self._folders_cache:
            self.marked_folders.add(folder)
        if not self.df_all.empty:
            self.marked_assets = set(pd.to_numeric(self.df_all["id"], errors="coerce").dropna().astype(int).tolist())
    ...
    def _selected_base_folder(self) -> Path:
        ...
        if self.selected_folder:
            return (DATA_DIR / self.selected_folder).resolve()
    ...
    def _safe_child_path(self, base_folder: Path, user_text: str, default_suffix: str = "") -> Optional[Path]:
        ...
        try:
            ...
            messagebox.showerror("Invalid Path", "Path must stay inside Data_Center/Data.")
            return None
    ...
    def new_data_file(self):
        ...
        allowed = {".csv", ".json", ".txt", ".md", ".db"}
        if suffix not in allowed:
            messagebox.showerror("Unsupported Type", "Supported: .csv, .json, .txt, .md, .db")
            return

        target = self._safe_child_path(base_folder, name, default_suffix=suffix)
        if target is None:
            return
            ...
            try:
                ...
                if suffix == ".csv":
                    with target.open("w", newline="", encoding="utf-8") as f:
                        writer = csv.writer(f)
                        writer.writerow(["date", "symbol", "value"])
                elif suffix == ".json":
                    target.write_text(json.dumps({"created_by": "Data Catalog", "items": []}, indent=2), encoding="utf-8")
                elif suffix == ".md":
                    target.write_text("# New Data Note\n\n", encoding="utf-8")
                elif suffix == ".txt":
                    target.write_text("", encoding="utf-8")
                elif suffix == ".db":
                    with sqlite3.connect(target) as conn:
                        conn.execute("CREATE TABLE IF NOT EXISTS metadata (key TEXT PRIMARY KEY, value TEXT)")
                        conn.execute("INSERT OR REPLACE INTO metadata (key, value) VALUES (?, ?)", ("created_by", "Data Catalog"))
                        conn.commit()
            except Exception as exc:
                messagebox.showerror("Create Data File Error", str(exc))
                return
        ...
        self.update_output()
        self.status_var.set(f"Created data file: {target}")
    ...
    def update_output(self):
        text = self.format_selected_tree() if (self.marked_folders or self.marked_assets) else self.format_tree(max_depth=2)
        ...
        def walk(folder: str, prefix: str, depth: int):
            ...
            for i, (kind, obj) in enumerate(entries):
                ...
                if kind == "folder":
                    ...
                    lines.append(f"{prefix}{con}{name} ({self.subtree_count(str(obj))})")
                    walk(str(obj), next_prefix, depth + 1)
                else:
                    name = safe_str(obj.get("file_name"))
                    typ = safe_str(obj.get("asset_type")) or file_type_from_name(name)
                    size = safe_str(obj.get("file_size_mb"))
                    lines.append(f"{prefix}{con}{name} | type={typ} | size={size}")

        walk(root, "", 0)
        return "\n".join(lines)

    def format_selected_tree(self) -> str:
        selected_files = set(self.marked_assets)
    ...
    def copy_path(self):
        ...
        if p is None:
            messagebox.showwarning("No selection", "Select a folder or asset first.")
            return
    ...
    def copy_relative_path(self):
        ...
        self.copy_to_clipboard(rel or "Data", "Copied relative path")

    def export_tree(self):
        p = filedialog.asksaveasfilename(defaultextension=".txt", initialfile="data_catalog_tree.txt", filetypes=[("Text files", "*.txt"), ("All files", "*.*")])
        if not p:
            return
    ...
    def selected_path(self) -> Optional[Path]:
        ...
        if row is not None:
            ...
            if str(p):
                return p
        if self.selected_folder:
            return DATA_DIR / self.selected_folder
        return DATA_DIR

    def open_file(self, asset_id: Optional[int] = None):
        if asset_id is not None:
            self.select_file(int(asset_id))
        p = self.selected_path()
    ...
    def show_help(self):
        messagebox.showinfo(
            "Data Catalog Tree",
            "Checkbox = mark/select\nDouble-click folder = open/close\nNew Folder = creates real folder in Data_Center/Data\nNew Data File = creates CSV/JSON/TXT/MD/DB file in Data_Center/Data.",
        )
    ...
    def show_menu(self):
        ...
        menu.add_command(label="New Data File", command=self.new_data_file)
        menu.add_separator()
    ...
    def show_tree_context_menu(self, event):
        ...
        menu.add_command(label="New Data File Here", command=self.new_data_file)
        menu.add_separator()
...
def build_panel(parent, repository: Optional[DataCatalogTreeRepository] = None, **kwargs):
    return DataCatalogTreeSelector(parent, repository=repository, **kwargs)


def create_panel(parent, repository: Optional[DataCatalogTreeRepository] = None, **kwargs):
    return build_panel(parent, repository=repository, **kwargs)


DashboardPanel = DataCatalogTreeSelector


if __name__ == "__main__":
    root = tk.Tk()
    root.title("QUANT WORKSPACE - Data Catalog Tree")
